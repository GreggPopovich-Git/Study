...
public class Exam_01 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			int endPage = nowpage * pageList;
			List<BoardVO> list = dao.getList(startPage, endPage);
			System.out.println("글번호\t작성자\t글제목\t조회수\t작성일자");
			for (int x = 0; x < list.size(); x++) {
				System.out.print(list.get(x).getBno() + "\t");
				System.out.print(list.get(x).getWriter() + "\t");
				System.out.print(list.get(x).getTitle() + "\t");
