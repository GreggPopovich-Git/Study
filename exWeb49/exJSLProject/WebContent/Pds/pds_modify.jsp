...
<script>
	function send() {
		...
		alert("글을 수정합니다.");
		pds.action = "pds_modify?page=<%= nowpage %>";
		pds.submit();
	}
	function cal() {
		...
		<section>
			...
			<input type="hidden" name="oldfilename" value="${vo.filename}">
				<table>
					...
					<tr>
						<th>제목</th>
						<td><input type="text" name="subject" maxlength="50" value="${vo.subject}" style="width:300px;height:30px;font-size:20px;"></td>
					</tr>
					<tr>
						<th>내용</th>
						<td><textarea rows="10" name="contents" id="contents" cols="60" maxlength="1000" onkeydown="cal()" onkeyup="cal()" onkeypress="cal()" style="overflow:auto; width:500px;height:300px;font-size:20px;">${vo.contents}</textarea><br>
						<input type="text" id="result" value="${length}" style="width:50px;height:30px;font-size:20px;border:none;background-color:#FBEFEF;" readonly><font style="font-size:20px;">/1000</font>
						</td>
					</tr>
					<tr>
		  				<th>파일첨부</th>
					...
					<tr>
						...
						<input type="button" value="수정하기" onClick="send()" style="width:300px;height:30px;font-size:20px;cursor:pointer;"> 
