...
public class PageIndex {
	public static String pageList(int page,int totpage,String url,String addtag, String sort, int idx) {
		...
		idx_pre = ((page-1) / 10);              // Priview 페이지 계산
	  	...
	  	// Prev 표시 부분
	  	if(idx_pre > 0) {
	  		s_pre = "<a href='"+url+"?page="+(idx_pre*10)+addtag+"&sort="+sort+"&idx="+idx+"'>"
	  	...
	  	for(int i=0;i<10;i++,idx_start++) {
	  		...
	  		if(idx_start == page)
	  			...
	  			s_idx = s_idx + " <a href='" + url + "?page=" + idx_start + "&sort=" + sort+"&idx="+idx;
	  			s_idx = s_idx + addtag + "'> " + idx_start + " </a> ";
	  		}
	  	}
		// Next 표시부분
	  	if(idx_start <= totpage ) {
	  		s_next = "<a href='"+url+"?page="+idx_start+addtag+"&sort="+sort+"&idx="+idx+"'>"
	...
	public static String pageListHan(int page,int totpage,String url,String query, String key, String sort) {
		...
		idx_pre = ((page-1) / 10);              // Priview 페이지 계산
	  	...
	  	// Prev 표시 부분
	  	if(idx_pre > 0) {
	  		s_pre = "<a href='"+url+"?page="+(idx_pre*10)+"&search="+query+"&key="+ URLEncoder.encode(key)+"&sort="+sort+"&row="+row+"'>"
	  	...
	  	for(int i=0;i<10;i++,idx_start++) {
	  		...
	  		if(idx_start == page)
	  			...
	  			s_idx = s_idx + "&search="+query+"&key=" + URLEncoder.encode(key) + "&sort=" + sort + "&row="+row+"'> " + idx_start + " </a> ";
	  		}
	  	}
		// Next 표시부분
	  	if(idx_start <= totpage ) {
