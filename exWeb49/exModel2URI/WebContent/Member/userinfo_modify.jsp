...
*/
	...
	//id 유효성 검사 입력시 사용된 문자검사(영문자, 숫자 이외의 다른 문자 사용 여부 검사)
	function idcheck(str){
		var useridExp = /^([a-zA-Z]{1})([a-zA-Z0-9]{5,10})$/;
		return useridExp.test(str);
	}	
/*
		...
		<tr><td align=center><br>                            
			<table cellpadding=0 cellspacing=0 border=0 width=650 align=center>       
							...
							<tr>
								...
								<TD BGCOLOR=WHITE>
									<table cellspacing=0 cellpadding=0>
										<tr>
											<td>
                  				<input type=text name=zip size=6 maxlength=6 readonly value="${user.zipcode}">
                  		</td>
                  		<td>
							...
							<tr>
								...
								<TD BGCOLOR=WHITE>
									<input type=text name=addr1 size=50 maxlength=100 readonly value="${user.addr1}">
								</td>
							</tr>
							<tr>
								<TD BGCOLOR="#EFF4F8">&nbsp;나머지 주소<font color=red>&nbsp;</font></td>
								<TD BGCOLOR=WHITE>
									<input type=text name=addr2 size=50 maxlength=100 value="${user.addr2}">
								</td>
							</tr>
							<tr>
              ...
              <tr>
				...
				<td bgcolor=WHITE valign=middle> 
					<input type="checkbox" name="fa" value="건강" <c:if test="${user.favorite.contains('건강')}"> checked </c:if>>건강 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="checkbox" name="fa" value="문화예술" <c:if test="${user.favorite.contains('문화예술')}"> checked </c:if>>문화예술 &nbsp;&nbsp;
					<input type="checkbox" name="fa" value="경제" <c:if test="${user.favorite.contains('경제')}"> checked </c:if>>경제 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="checkbox" name="fa" value="연예오락" <c:if test="${user.favorite.contains('연예오락')}"> checked </c:if>>연예오락 &nbsp;
					<input type="checkbox" name="fa" value="뉴스" <c:if test="${user.favorite.contains('뉴스')}"> checked </c:if>>뉴스
					<br><input type="checkbox" name="fa" value="여행레져" <c:if test="${user.favorite.contains('여행레져')}"> checked </c:if>>여행레져 &nbsp;&nbsp;
					<input type="checkbox" name="fa" value="생활" <c:if test="${user.favorite.contains('생활')}"> checked </c:if>>생활 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="checkbox" name="fa" value="스포츠" <c:if test="${user.favorite.contains('스포츠')}"> checked </c:if>>스포츠 &nbsp;&nbsp;
					<input type="checkbox" name="fa" value="교육" <c:if test="${user.favorite.contains('교육')}"> checked </c:if>>교육 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="checkbox" name="fa" value="컴퓨터" <c:if test="${user.favorite.contains('컴퓨터')}"> checked </c:if>>컴퓨터 &nbsp;&nbsp;
					<input type="checkbox" name="fa" value="학문" <c:if test="${user.favorite.contains('학문')}"> checked </c:if>>학문 &nbsp;&nbsp;
				</td>
              </tr>
              <tr>
                ...
                <TD BGCOLOR=WHITE>
                  <select name=job class="formbox">
 					...
 					<option value="회사원" <c:if test="${user.job=='회사원'}">selected </c:if>>회사원
 					<option value="연구전문직" <c:if test="${user.job=='연구전문직'}">selected </c:if>>연구전문직
 					<option value="교수학생" <c:if test="${user.job=='교수학생'}">selected </c:if>>교수학생
 					<option value="일반자영업" <c:if test="${user.job=='일반자영업'}">selected </c:if>>일반자영업
 					<option value="공무원" <c:if test="${user.job=='공무원'}">selected </c:if>>공무원
 					<option value="의료인" <c:if test="${user.job=='의료인'}">selected </c:if>>의료인
 					<option value="법조인" <c:if test="${user.job=='법조인'}">selected </c:if>>법조인
 					<option value="종교,언론,에술인" <c:if test="${user.job=='종교,언론,에술인'}">selected </c:if>>종교.언론/예술인
 					<option value="농,축,수산,광업인" <c:if test="${user.job=='농,축,수산,광업인'}">selected </c:if>>농/축/수산/광업인
 					<option value="주부" <c:if test="${user.job=='주부'}">selected </c:if>>주부
 					<option value="무직" <c:if test="${user.job=='무직'}">selected </c:if>>무직
 					<option value="기타" <c:if test="${user.job=='기타'}">selected </c:if>>기타
				   </select>
								</td>                
              </tr>
              <tr>
                ...
                <TD BGCOLOR=WHITE>
                  <textarea cols=65 rows=5 name="intro">${user.intro }</textarea>
                </td>
              </tr>
            </table>
            <table cellpadding=0 cellspacing=0 border=0 width=100%>
